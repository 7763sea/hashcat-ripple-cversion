...
//---------------------------------------------------------
	  u32 encode_step = 24*sizeof(u32);
	  
	 // u32 * global_all_encode_buf =(u32 *)malloc(encode_step*(pws_cnt));
	  
//---------------------------------------------------------	  
      for (u32 loop_pos = 0, slow_iteration = 0; loop_pos < iter; loop_pos += loop_step, slow_iteration++)
      {
		 //// printf("loop count ------------------------ highest_pw_len = %d  iter = %d loop_step= %d pws_cnt =%d  slow_iteration= %d  \r\n",highest_pw_len,iter,loop_step,pws_cnt,slow_iteration);
		...
		//------------------------------------------------------------------------	
		//pw_t all_pw_buf[pws_cnt]; 
		pw_t* all_pw_buf = (pw_t*)malloc(pws_cnt*sizeof(pw_t)); 
		//pbkdf2_sha256_tmp_t all_tmps_buf[pws_cnt];
		pbkdf2_sha256_tmp_t *all_tmps_buf = (pbkdf2_sha256_tmp_t *)malloc(pws_cnt*sizeof(pbkdf2_sha256_tmp_t));
		int CL_rc = hc_clEnqueueReadBuffer (hashcat_ctx, device_param->command_queue,device_param->d_tmps, CL_TRUE, 0, sizeof (pbkdf2_sha256_tmp_t)*pws_cnt, all_tmps_buf, 0, NULL, NULL);
		//printf("%08X ,%08X,%08X,%08X,%08X,%08X,%08X,%08X\r\n",all_tmps_buf[0].out[0],all_tmps_buf[0].out[1],all_tmps_buf[0].out[2],all_tmps_buf[0].out[3],all_tmps_buf[0].out[4],all_tmps_buf[0].out[5],all_tmps_buf[0].out[6],all_tmps_buf[0].out[7]);
		if (CL_rc == -1) return -1;
		int rc = hc_clEnqueueReadBuffer (hashcat_ctx, device_param->command_queue,device_param->d_pws_buf, CL_TRUE, 0, sizeof (pw_t)*pws_cnt, all_pw_buf, 0, NULL, NULL);
		//printf("%08X ,%08X,%08X,%08X,%08X,%08X,%08X,%08X\r\n",tmps.out[0],tmps.out[1],tmps.out[2],tmps.out[3],tmps.out[4],tmps.out[5],tmps.out[6],tmps.out[7]);
		if (rc == -1) return -1;
		pw_t pw;
		 rc = gidd_to_pw_t (hashcat_ctx, device_param, 0, &pw);
		if (rc == -1) return -1;
		...
		{						
			index_encode = all_tmps_buf[count_gid].out[0];
			memcpy(all_encode_buf+count_gid*(24),encode_split,4*sizeof(u32));
			memcpy(all_encode_buf+count_gid*(24)+4,all_pw_buf[count_gid].i,all_pw_buf[count_gid].pw_len*sizeof(u32));
			//printf("this is debug ------------------------------02---01 count_gid %d\n",count_gid);

			memcpy((all_encode_buf)+count_gid*(24)+12,all_tmps_buf[count_gid].out,8*sizeof(u32));
			//printf("this is debug ------------------------------02---03  count_gid =%d\n",count_gid);
			//memcpy((all_encode_buf)+count_gid*(24)+20,encode_end,4*sizeof(u32));
			...
			char all_path[106];
			char * path = "./pass/";
			itoa(index_encode,file_name,10);
			sprintf(all_path,"%s%s",path,file_name);
			FILE *fp = fopen (all_path, "wb");

			if (fp == NULL)
			{
			event_log_error (hashcat_ctx, "%s: %s", "123", strerror (errno));
			...
			fwrite(all_encode_buf,sizeof(u32),(24)*pws_cnt,fp); 
...
int run_kernel_tm (hashcat_ctx_t *hashcat_ctx, hc_device_param_t *device_param)
{
  ...
  const u64 kernel_threads = device_param->kernel_threads_by_wgs_kernel_tm;
...
int run_kernel_amp (hashcat_ctx_t *hashcat_ctx, hc_device_param_t *device_param, const u64 num)
{
  ...
  const u64 kernel_threads = device_param->kernel_threads_by_wgs_kernel_amp;
...
int run_kernel_memset (hashcat_ctx_t *hashcat_ctx, hc_device_param_t *device_param, cl_mem buf, const u32 value, const u64 size)
{
  ...
  if (num16d)
  {
    ...
    const u64 kernel_threads = device_param->kernel_threads_by_wgs_kernel_memset;
